        ...
        (if (not (re-search-forward re (line-end-position) t))
            (progn
              ...
              (if (eq (line-end-position) (point-max))
                ...
                (forward-line)
                (beginning-of-line)
                (setq current-indent (+ current-indent current-indent-diff)
                      current-indent-diff 0)))
          (let* ((matched-group (cl-find-if #'match-string-no-properties groups))
                 (found-literal (cdr (assoc matched-group group-lookup))))
            (cond ((memq found-literal opening)
                   (cl-incf current-indent-diff xquery-mode-indent-width))
                  ((memq found-literal closing)
                   (cl-decf current-indent-diff xquery-mode-indent-width)))))))))
