...
be indented."
  (cond
   ...
   ((previous-line-starts-with "^\\s-*\\<if\\>")
    (previous-line-indentation))
   ((previous-line-starts-with "^\\s-*\\<then\\>")
    (+ (previous-line-indentation) xquery-mode-indent-width))
      ...
      (while (not exit)
        ...
        (if (not pos)
          ...
          (let ((last-match (car (delq nil (mapcar #'match-string-no-properties '(1 2 3 4 5 6))))))
            (cond ((eq (point) (line-beginning-position))
                   (push (list (line-number-at-pos)) matches)
                   (if (< (point) limit)
                       (forward-char)
                     (setq exit t)))
                  ((looking-back open-re nil)
                   (push (list last-match (line-number-at-pos)) matches))
